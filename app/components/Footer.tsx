            ...
            <div className="flex justify-center text-2xl">
              {LINKS.map((link) => (
                <div key={link.id} className="mx-5 hover:animate-pulse">
                    <Link href={link.url}>
                      <a>
                        {link.name === 'Linkedin' && <FaLinkedin className="text-blue-700"/>}
                        {link.name === 'Malt' && <img src="/icons/malt.png" width={25} height={25}/>}
                        {link.name === 'Github' && <VscGithub/>}
                        {link.name === 'DockerHub' && <FaDocker className="text-blue-400"/>}
                      </a>
                    </Link>
                </div>
              ))}
            ...
            <Link href={`mailto:${EMAIL}`}>
              <a>
                <button className="pushable">
                  <span className="pushableFront">
                    {FOOTER[langId].contact}
                  </span>
                </button>
              </a>
            </Link>
